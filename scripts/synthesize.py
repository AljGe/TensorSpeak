...
def main() -> None:
    ...
    pipeline = InflectPipeline(provider=args.provider)
    load_ms = (time.perf_counter() - started) * 1000.0
    print(f"sessions loaded in {load_ms:.0f} ms ({args.provider})")
    ...
    for index, t in enumerate(result.traces):
        ...
        print(f"  mel_len     : {t.mel_len}   -> m_p_exp/logs_p_exp [1, 192, {t.mel_len}]")
